...
UNITY_DOTS_INSTANCING_START(MaterialPropertyMetadata)
	...
	UNITY_DOTS_INSTANCED_PROP(float4, _PositionTexture_ST)
	UNITY_DOTS_INSTANCED_PROP(float4, _NormalTexture_ST)
	UNITY_DOTS_INSTANCED_PROP(float4, _AnimMeta_ST) 
	UNITY_DOTS_INSTANCED_PROP(float, _InvAnimCount)
	//UNITY_DOTS_INSTANCED_PROP(float, _AnimIndex)
...
//
//// Fallback uniform ONLY when no instancing mode is active
//#if !defined(UNITY_INSTANCING_ENABLED) && !defined(UNITY_DOTS_INSTANCING_ENABLED)
//// (or: #if !defined(UNITY_ANY_INSTANCING_ENABLED) on newer URP)
//float _AnimIndex;
//#endif// Fallback for non-instanced variants so they compile
//#ifndef UNITY_INSTANCING_ENABLED
//float _AnimIndex;   // material-uniform fallback when INSTANCING_ON is off
//#endif
...
static float  unity_DOTS_Sampled_InvAnimCount;
//static float  unity_DOTS_Sampled_AnimIndex;
...
void SetupDOTSLitMaterialPropertyCaches()
{
    ...
    unity_DOTS_Sampled_InvAnimCount         = UNITY_ACCESS_DOTS_INSTANCED_PROP_WITH_DEFAULT(float , _InvAnimCount);
    //unity_DOTS_Sampled_AnimIndex            = UNITY_ACCESS_DOTS_INSTANCED_PROP_WITH_DEFAULT(float , _AnimIndex);
...
#define _InvAnimCount           unity_DOTS_Sampled_InvAnimCount
//#define _AnimIndex              unity_DOTS_Sampled_AnimIndex

#endif

TEXTURE2D_ARRAY(_PositionTexture);
SAMPLER(sampler_PositionTexture);

TEXTURE2D_ARRAY(_NormalTexture);
SAMPLER(sampler_NormalTexture);

TEXTURE2D(_AnimMeta);
SAMPLER(sampler_AnimMeta);
